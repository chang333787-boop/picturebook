...
> 입력: AI_MASTER_PLAN_CLAUDE_v3 (합의안 — 일부 흐름 폐기), **AI_POLICY_V140.md (새 정책 박힘)**
> 시점: 2026-05-20
> 위치: `/Users/dobuk/Downloads/picturebook-repo/AI_DECISIONS_FINAL.md`
> 상태: **실행용 — 사용자 최종 결정 박는 문서**

## ⚠️ v140 정책 박힘 (2026-05-20) — 핵심 변경

`AI_POLICY_V140.md` 박힌 새 정책 박혔음. 다음 7가지가 v3 박힌 거에서 변경:

1. **운영 모드 ↔ 테스트 모드 분리** — Phase 0.5 mock은 테스트 모드 (마감/quota 우회 가능)
2. **원본 최종 마감 원칙** — 운영에서 마감 후만 AI 가능
3. **교사 AI 허용제** — 기본 OFF, 교사 ON 박혀야
4. **브랜치 lineage / copyDepth** — 0/1까지 AI, 자식 재복사 X
5. **aiVariants 토글 저장** — `_rtSaveBody` 폐기, 원본 보존 + AI 별도 저장
6. **텍스트 1단계 후보 3회** — generating/candidate_ready/drafting/finalized
7. **다층 quota 방어** — 브랜치 + rootBranchId + Functions hard cap

이 문서 아래 박힌 내용 중 v140과 충돌하는 거는 v140 우선.

---

# 0. 이 문서의 역할

- v3 합의안에서 박힌 결정 항목 28개 + **v140 새 결정 항목 6개** 분류
- **구현 금지 조건** 명시
- **Phase 0 진행 조건** 5개
...
## 2-5. v140 새 결정 항목 (2026-05-20 박힘)

`AI_POLICY_V140.md` 박힘으로 새로 생긴 결정/보류 항목.

| # | 항목 | 상태 |
|---|---|---|
| 29 | 운영 모드 ↔ 테스트 모드 분리 | ✓ 확정 — v140 박힘 |
| 30 | 원본 최종 마감 원칙 (운영) | ✓ 확정 — 마감 후 AI만 가능 |
| 31 | 교사 AI 허용제 (기본 OFF) | ✓ 확정 — `aiPermission.enabled` 박힘 |
| 32 | 브랜치 lineage / `copyDepth` 0/1 AI 허용 | ✓ 확정 — depth 2+ 금지 |
| 33 | 자식 브랜치 재복사 금지 | ✓ 확정 — depth 1 복사 버튼 비활성화 |
| 34 | aiVariants 토글 저장 (`_rtSaveBody` 폐기) | ✓ 확정 — 원본 보존 + 별도 저장 흐름 |
| 35 | 텍스트 1단계 후보 3회 + 미세 수정 + 마감 | ✓ 확정 — 5 상태값 박힘 |
| 36 | 다층 quota 방어 (브랜치 + rootBranchId + hard cap) | ✓ 확정 |
| 37 | 작품 검사 마감 전/후 허용 | ⚠️ **보류** — 추천 = 마감 전 가능 (진단만이라) |
| 38 | rootBranchId 묶음 quota 값 | ⚠️ **보류** — 베타에서 결정 |
| 39 | 한 번에 생성 가능 복사본 개수 상한 | ⚠️ **보류** — 초기 추천 10개 |
| 40 | 이미지 AI 장면당 quota | ⚠️ **보류** — Phase D·E에서 박음 |
| 41 | aiDrafts TTL 기간 | ⚠️ **보류** — 마감 후 24h? 7일? |
| 42 | testMode 진입 조건 | ⚠️ **보류** — 교사 계정만? URL ?test=1? |
...
- [x] Phase 0.5 mock 박힘 (v140 4 step + 5 fix, 2026-05-20~21) — **✅ 사용자 점검 OK 박힘 (2026-05-21)** — 15 체크리스트 모두 합격
