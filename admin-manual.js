  ...
  var TEACHER = {
    ...
    sections: [
      { n: 1, h: '교사 계정 만들기', steps: [
        '첫 화면에서 “수업 관리”로 들어가요. (주소로는 maker.html?admin=1)',
      ...
      { n: 5, h: '학생 작품 관리하기 (공개·수정·감상)', steps: [
        '팀 목록에서 모둠마다 🛠 수정·▶️ 감상, ⋯ 메뉴의 🖨 인쇄를 쓸 수 있어요. 선생님은 PIN 없이 바로 편집돼요.',
        '작품을 “공개(발행)”하면 감상 화면(뷰어)에서 볼 수 있어요. ▶️ 감상 버튼은 작품이 감상 가능 상태(🟢)일 때 켜지고, 공개 전에는 🛠 수정으로 들어가 내용을 볼 수 있어요.',
        '🛠 수정으로 들어가 글·그림을 직접 고치고, ▶️ 감상으로 완성한 이야기를 처음부터 읽어볼 수 있어요.',
        '편집 화면에서 왼쪽 위 “← 관리로”를 누르면 다시 관리 화면으로 돌아와요.',
      ] },
      { n: 6, h: '인쇄물 (그림책·나침반·고쳐쓰기)', steps: [
        '그림책 인쇄: 장면을 화면 그대로(그림·말풍선) 책처럼 뽑아요. 팀 카드의 ⋯ 메뉴에서 열어요.',
        '생각 나침반 결과·고쳐쓰기 자료도 인쇄할 수 있어요 — 작품마다 모두 뽑을 수 있어요.',
        '입장 카드·이 설명서도 인쇄할 수 있어요.',
