...
function _renderDocsTab() {
  ...
  host.innerHTML = `
    ...
    <div class="admin-tc">
      <div class="admin-tc-head">
        <div class="admin-tc-title">📝 개인정보 수집·이용 및 생성형 AI 활용 동의서 (가정 배부용)</div>
        <div class="admin-tc-desc">
          <b>가지는 학생 개인정보를 수집하지 않아요</b>(모둠 이름·PIN만 사용). 이 양식은 학급에서 <b>따로</b>
          이름·사진·작품을 모으려는 선생님을 위한 A4 한 장 서식이에요. <b>수업 중 생성형 AI 사용에 대한
          보호자 동의(②)</b>도 함께 들어 있어요. 필요한 항목만 ☑ 표시해 인쇄한 뒤 가정으로 보내고 서명을 받아 보관하세요.
        </div>
      </div>
      <div class="admin-docs-actions">
...
/* ════════════════════════════════════════════════════════════════
   ...
   CONSENT-FORM-2(2026-09-06) — 생성형 AI 활용 안내 블록 + 동의 ②(AI) 추가. 전국대회 컨설팅 지적
     ("AI 접근 동의가 동의서에 있어야") 대응. 문구는 Desktop docx(보호자용)·privacy.html 6절과 동일 취지.
   ─────────────────────────────────────────────────────────────────
...
function _consentFormHtml() {
  /* CONSENT-FORM-3: 본문은 consent-form.js(window.ConsentForm) 단일 출처 — 심사위원 교사 화면 미리보기(consent.html)와 공유 */
  if (window.ConsentForm && typeof window.ConsentForm.html === 'function') return window.ConsentForm.html();
  return '<div class="cf-doc"><p>동의서 서식을 불러오지 못했어요. 새로고침 후 다시 시도해 주세요.</p></div>';
}
function _printConsentForm() {
  ...
  const style = document.createElement('style');
  style.id = STYLE_ID;
  style.textContent = (window.ConsentForm && window.ConsentForm.printCss) ? window.ConsentForm.printCss(ROOT_ID) : '';
  document.head.appendChild(style);
  try {
