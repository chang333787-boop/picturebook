...
test('planImageS2Batch — promptVersion 다르면(P4→P5) cached 아님 → target', () => {
  const scenes = { 1: { imageData: 'a' }, 2: { imageData: 'b' } };
  const ev = {
    1: { url: 'u', stale: false, basedOnImageHash: 'h1', promptVersion: 'imgS2-p4-OLD' },   /* 이전 버전 → 재생성 */
    2: { url: 'u', stale: false, basedOnImageHash: 'h2', promptVersion: 'imgS2-p5-v1' },     /* 최신 → cached */
  };
  const fps = { 1: 'h1', 2: 'h2' };
  const p = B.planImageS2Batch({ scenes, existingVariants: ev, fingerprints: fps, currentPromptVersion: 'imgS2-p5-v1' });
