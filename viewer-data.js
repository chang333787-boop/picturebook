...
async function loadTeamData(teamName, classId = null, fromMaker = false, ptypeHint = null) {
  ...
  if (meta) {
    ...
    /* LV1-WAIT-1(2026-09-05): 1단계 완성 대기화면 — 생성 중이면 재부착(서버 상태 기반), 완성이면 조용히. */
    try { setTimeout(_maybeMountLv1BookWait, 400); } catch (e) { /* noop */ }
...
/* LV1-WAIT-1(2026-09-05): 종전 이 자리의 1단계 진행 배지(_maybeStartLv1ImageBadge·8초 폴링)는 폐기 —
   lv1-book-wait.js(공용 대기화면)가 aiVariants 구독으로 대체한다. 제작자 세션(fromMaker)에서만 mount,
   데이터 로드 시점엔 allowPrompt:false(선택/그리기 카드는 튜토리얼 뒤 __maybeShowLevel2CharGate가 허용). */
function _maybeMountLv1BookWait() {
  try {
    const p = ViewerState.project || {};
    if (!(p.projectType === 'picturebook' && p.picturebookLevel === 1)) return;
    if (!ViewerState.fromMaker) return;
    if (!p.classId || !p.teamName) return;
    if (window.Lv1Book && typeof window.Lv1Book.mountIfNeeded === 'function') {
      window.Lv1Book.mountIfNeeded({ classId: p.classId, teamName: p.teamName }, { allowPrompt: false, page: 'viewer' });
    }
  } catch (e) { /* noop */ }
}
